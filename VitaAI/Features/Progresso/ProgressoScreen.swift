...
struct ProgressoScreen: View {
    ...
    private func badgeTile(_ badge: BadgeWithStatus) -> some View {
        ...
        return VStack(spacing: 6) {
            ...
            Text(badge.name)
                .font(.system(size: 10, weight: .medium))  // ds-allow: tela de stats — token debt herdado (tokenizar no gold do Progresso #33)
                .foregroundStyle(color)
                .lineLimit(1)
                .multilineTextAlignment(.center)
        }
    ...
    private func activityRow(_ item: ActivityFeedItem) -> some View {
        HStack(spacing: 12) {
            Text(item.action)
                .font(.system(size: 12, weight: .medium))  // ds-allow: tela de stats — token debt herdado (tokenizar no gold do Progresso #33)
    ...
    private func statCard(value: String, label: String, valueColor: Color) -> some View {
        glassCard {
            VStack(alignment: .leading, spacing: 4) {
                Text(value)
                    .font(.system(size: 20, weight: .bold))  // ds-allow: tela de stats — token debt herdado (tokenizar no gold do Progresso #33)
    ...
    private func weakAreaRow(name: String, meta: String, pct: Int, color: Color) -> some View {
        HStack(spacing: 10) {
            ...
            Text(String(name.prefix(2)).uppercased())
                ...
                .background(
                    RoundedRectangle(cornerRadius: 8)  // ds-allow: tela de stats — token debt herdado (tokenizar no gold do Progresso #33)
